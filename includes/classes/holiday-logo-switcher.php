	...
	public function whls_shortcode() {
        ...
        if ( $query->have_posts() ) {
            while ( $query->have_posts() ) {
				...
				$date_from = strtr( get_post_meta( get_the_ID(), 'whls_options_from_date_', true ), '/', '-' );
				$date_from = date( 'Y-m-d', strtotime($date_from) );
				$date_to = strtr( get_post_meta( get_the_ID(), 'whls_options_to_date_', true ), '/', '-' );
				$date_to = date( 'Y-m-d', strtotime($date_to) );
				$image_alt = get_post_meta( get_the_ID(), 'logo_options_image_alt', true );

				//var_dump($date_today); //2022-01-03
				//var_dump($date_from); //1970-01-01
				//var_dump($date_to); //2022-12-31

				if ( ($date_today >= $date_from) && ($date_today <= $date_to ) ) {
					$img = '<a href="' . get_home_url() . '" id="whls__logo whls__logo--custom"><img src="' . get_the_post_thumbnail_url() . '" alt="' . $image_alt . '"/></a>';
				}
			}
		}
		$result .= $img;
